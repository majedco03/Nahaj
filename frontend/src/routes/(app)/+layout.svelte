<script lang="ts">
	...
	import NotificationPoller from '$lib/components/nahaj/NotificationPoller.svelte';
	...
	const nahajRoutePrefixes = ['/courses', '/plan', '/calendar', '/quizzes'];

	$: isNahajScreen = nahajRoutePrefixes.some(
			...
			{:else}
				...
				{#if loaded}
					<main
						id="main-content"
						class={isNahajScreen
							? 'flex min-w-0 w-full max-w-full flex-1 h-screen max-h-[100dvh] flex-col @container transition-[max-width] duration-200 ease-in-out'
							: 'contents'}
						style:max-width={isNahajScreen && $showSidebar && !$mobile
							? 'calc(100% - var(--sidebar-width))'
							: undefined}
					>
						{#if isNahajScreen && $mobile}
							<nav class="shrink-0 px-2.5 pt-2 pb-1 backdrop-blur-xl drag-region">
								<Tooltip
								...
								>
									<button
										...
										type="button"
										class="flex size-7 cursor-pointer items-center justify-center rounded-lg transition hover:bg-gray-100 dark:hover:bg-gray-850"
