<script lang="ts">
	...
	// Local attachments are stored in Nahaj's course library when this hook is
	// provided, keeping Open WebUI's generic file store out of the workflow.
	export let nahajUpload: ((file: File) => Promise<any>) | null = null;
	...
	const uploadFileHandler = async (file, process = true, itemData = {}) => {
		...
		if (nahajUpload) {
			...
			try {
				const document = await nahajUpload(file);
				if (!document?.id) throw new Error('Nahaj did not return a document ID.');
				fileItem.status = 'uploaded';
				fileItem.file = document;
				fileItem.id = document.id;
				fileItem.url = document.id;
				fileItem.nahaj_document_id = document.id;
				fileItem.collection_name = document.collection;
				fileItem.content_type = document.media_type;
				fileItem.size = document.size_bytes ?? file.size;
				files = files;
				toast.success($i18n.t('File added to the Nahaj course library'));
			} catch (e) {
	...
	const inputFilesHandler = async (inputFiles) => {
		...
		inputFiles.forEach(async (file) => {
			...
			if (file['type'].startsWith('image/')) {
				if (!nahajUpload && visionCapableModels.length === 0) {
				...
				reader.onload = async (event) => {
					...
					if ($temporaryChatEnabled && !nahajUpload) {
						files = [
							...files,
							{
								type: 'image',
						...
						>
							...
							<div class=" flex justify-between mt-0.5 mb-2 mx-0.5 max-w-full" dir="ltr">
								<div class="ml-1 self-end flex items-center flex-1 min-w-0">
									...
									fileUploadCapableModels={nahajUpload ? selectedModelIds : getFilesystemUploadTerminal(
											$selectedTerminalId,
											$terminalServers,
											$settings
										)
