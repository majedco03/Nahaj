<script lang="ts">
	...
	let params = {};
	let chatVariables = {};
	let nahajCourses: Course[] = [];
	let selectedNahajCourseId = '';
	let selectedNahajCollection: Collection = 'slides';
	let nahajDocumentIds: string[] = [];
	$: nahajDocumentIds = [...files, ...chatFiles]
		.map((file) => file?.nahaj_document_id)
	...
	onMount(() => {
		...
		const init = async () => {
			...
			const nahajDraft = window.localStorage.getItem('nahaj-chat-draft');
			if (nahajDraft) {
				window.localStorage.removeItem('nahaj-chat-draft');
				prompt = nahajDraft;
				messageInput?.setText(nahajDraft);
			}
	...
	const uploadGoogleDriveFile = async (fileData) => {
		...
		try {
			...
			// Course material always belongs to Nahaj. The generic Open WebUI file
			// endpoint is retained only for non-Nahaj integrations in upstream code.
			console.log('Uploading file to server...');
			const uploadedFile = uploadNahajFile
				? await uploadNahajFile(file)
				: await uploadFile(localStorage.token, file, metadata);
			...
			fileItem.collection_name = uploadNahajFile
				? uploadedFile.collection
				: uploadedFile?.meta?.collection_name;
			if (uploadNahajFile) {
				fileItem.nahaj_document_id = uploadedFile.id;
				fileItem.content_type = uploadedFile.media_type;
				fileItem.size = uploadedFile.size_bytes ?? file.size;
			}
	...
	const onUpload = async (event) => {
		...
		if (type === 'google-drive') {
			...
			toast.error('Web attachments are not part of the Nahaj course library. Upload a course file instead.');
		}
	};

	const uploadNahajFile = async (file: File) => {
		if (!selectedNahajCourseId) {
			throw new Error('Choose a course before uploading course material.');
		}
		return uploadDocument(selectedNahajCourseId, selectedNahajCollection, file);
	};
	...
	const sendMessageSocket = async (
		...
		const res = await generateOpenAIChatCompletion(
			localStorage.token,
			{
				...
				// Nahaj documents are already stored by the domain API. Do not send them
				// through Open WebUI's generic file store; the supervisor receives IDs in
				// the explicit context field below.
				files: (files?.filter((file) => !file?.nahaj_document_id).length ?? 0) > 0
					? files.filter((file) => !file?.nahaj_document_id)
					: undefined,
				...
				nahaj_context: {
					course_id: selectedNahajCourseId || null,
					document_ids: nahajDocumentIds
				},
				variables: {
					...getPromptVariables(
						$user?.name,
		...
		<div in:fade={{ duration: 50 }} class="w-full h-full flex flex-col">
			...
			<div class="w-full h-full flex">
				...
				{:else}
					...
					>
						{#if !embedded}
							<div class="mb-2 flex items-center justify-end px-1">
						...
						{:else}
							<div class="flex items-center h-full">
								<Placeholder
									...
									{onUpload}
									nahajUpload={uploadNahajFile}
									nahajCourses={nahajCourses}
									bind:selectedNahajCourseId
									bind:selectedNahajCollection
									{onUpdate}
									messageQueue={$chatRequestQueues[$chatId] ?? []}
									askUser={savedAskUserPrompt ?? socketAskUserPrompt}
									onQueueSendNow={sendQueuedMessageNow}
