<script lang="ts">
	...
	// Nahaj keeps the primary navigation intentionally small.  Chat remains the
	// shell's first item; these four domain routes are the only pinned screens.
	const DEFAULT_PINNED_ITEMS = ['courses', 'plan', 'calendar', 'quizzes'];
	...
	const getMenuItemMeta = (id) => {
		const items = {
			courses: { label: 'Courses', href: '/courses', iconType: 'workspace' },
			plan: { label: 'Plan', href: '/plan', iconType: 'check' },
			notes: { label: 'Notes', href: '/notes', iconType: 'note' },
			workspace: { label: 'Workspace', href: '/workspace', iconType: 'workspace' },
			automations: { label: 'Automations', href: '/automations', iconType: 'automations' },
			calendar: { label: 'Calendar', href: '/calendar', iconType: 'calendar' },
			quizzes: { label: 'Quizzes', href: '/quizzes', iconType: 'clock' },
						...
						<div id="pinned-menu-items-list">
							...
							<div class="flex self-center translate-y-[0.5px]">
								<div class=" self-center text-[0.8125rem] leading-5">
									{$i18n.t(meta.label)}
									{#if itemId === 'plan' && $nahajUnreadNotifications > 0}
										<span class="ml-1 rounded-full bg-indigo-600 px-1.5 py-0.5 text-[0.6rem] text-white">{$nahajUnreadNotifications}</span>
									{/if}
								</div>
											</div>
										</a>
